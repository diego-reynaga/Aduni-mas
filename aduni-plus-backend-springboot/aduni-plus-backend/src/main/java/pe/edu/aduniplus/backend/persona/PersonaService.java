...
public class PersonaService {
    ...
    public PersonaResponse crearPersona(PersonaRequest request) {
        ...
        if (existingPersona != null) {
            ...
            String discriminator = discriminatorFor(targetType);
            ...
            entityManager.createNativeQuery("UPDATE persona SET tipo_persona = :tipo, nombre_persona = :nombres, apellido_persona = :apellidos, telefono = :telefono, correo_persona = :correo WHERE id = :id")
                .setParameter("tipo", discriminator)
    ...
    private void insertarHijoNativo(Long id, String tipo, PersonaRequest request) {
        switch (tipo) {
            case "ADMINISTRATIVO" -> {
                entityManager.createNativeQuery("INSERT INTO administrativo (persona_id, activo, cargo, codigo_administrativo) VALUES (:id, true, :cargo, :codigo)")
            ...
            case "DOCENTE" -> {
                entityManager.createNativeQuery("INSERT INTO docente (persona_id, activo, area_academica, codigo_docente, especialidad) VALUES (:id, true, :area, :codigo, :especialidad)")
            ...
            case "ESTUDIANTE" -> {
                entityManager.createNativeQuery("INSERT INTO estudiante (persona_id, activo, codigo_estudiante) VALUES (:id, true, :codigo)")
                    .setParameter("id", id).setParameter("codigo", request.codigo()).executeUpdate();
            }
            case "PADRE_FAMILIA" -> {
                entityManager.createNativeQuery("INSERT INTO apoderado (persona_id, activo, ocupacion) VALUES (:id, true, :ocupacion)")
                    .setParameter("id", id).setParameter("ocupacion", request.ocupacion()).executeUpdate();
            }
        }
    }

    private String discriminatorFor(String tipoPersona) {
        return "PADRE_FAMILIA".equalsIgnoreCase(tipoPersona) ? "APODERADO" : tipoPersona;
    }
    ...
    public void eliminarPersona(Long id) {
        ...
        if (usuarioRepository.findByUsername(persona.getCorreo()).isPresent() ||            usuarioRepository.findAll().stream().anyMatch(u -> u.getPersona().getId().equals(id))) {
